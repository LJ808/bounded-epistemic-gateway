...
Three items named above as undone work now carry real, checked results, not further promises.

Fallacy coverage now spans all twenty-six categories the original submission's screen tracks. Double-counting joined the bounds-pair pass in a later session, closing the prior one-of-twenty-six gap.

Propagation now iterates to a genuine fixed point (`propagate_to_fixed_point()`) rather than running single-pass only, confirmed via a separate `check_order_independence()` check that reaches the same result regardless of edge-processing order. On the twelve-claim graph this project builds from, single-pass already matched the fixed point exactly — a real finding about this graph's density, not a claim that holds for every graph shape.

The ground-truth question resolved differently than optimism might predict. A real check ran (`imst_ground_truth_calibration_test.py`), using IMST's own settled trading outcomes — no hand-labeling, real market resolution — as ground truth against one thesis: whether a sustained race lead predicts a driver's win. The current OR-combine formula converges to a confident known-true against a real 25% win rate — a genuine miscalibration, not a validation. An AND-combine alternative avoids that overclaim but produces no magnitude match either, landing at the neutral prior regardless of the real win rate underneath it.

A multi-graph extension (`multi_graph_calibration_test.py`) followed the same session — the machinery now generalizes to any number of real theses, but exactly one real graph with settled ground truth exists in this vault as of this writing. A second real graph (World Cup mutex-arb convergence, late-goal repricing, or favorite-fade — all speced, all blocked on a real settled-outcome join not yet pulled) would let the study run at the scale it targets. Nothing here fabricates a second graph to look more complete than the real state supports.

What stays genuinely open, not resolved by any of the above: the OR-combine formula now carries a real, reproduced failure on real data, not just an unvalidated design choice. Whether it needs replacing, and with what, remains an unmade decision — the AND alternative avoids the specific overclaim found here but brings its own failure to represent "mostly wrong, occasionally right" cases, and neither formula has run against enough real graphs yet to call either one calibrated in general.
