...
- [[literature-engagement-addendum-v2]], [[literature-engagement-addendum-v3]], [[literature-engagement-addendum-v4]] — three further outside papers engaged the same way: a third source, Artificial Hivemind's judge-miscalibration finding wired into a real `review_priority()` flag and tested across six real local models; a fourth source, Sourbut & Goldhaber's own three-layer framework, confirmed as the direct, previously-unread origin of language this project's reviewer had already been using in feedback
- [[literature-engagement-addendum-v5]] — a fifth source, on LLM pretraining data-poisoning (Souly et al., arXiv:2510.07192). States plainly that the paper's training-time threat model doesn't apply to this project's own architecture (the Gateway trains nothing), then proposes one real, unrun extension: using the Politics/Trumpian folder's newly source-labeled documents (academic through movement-propaganda tiers) to test whether the Gateway's own scoring call responds to a source's actual epistemic quality or only to how confidently a claim gets phrased
